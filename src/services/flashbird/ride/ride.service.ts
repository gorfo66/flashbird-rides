...
export class RideService {
  ...
  public getRides(): Observable<Ride[]> {
    return this.httpClient.post<GetRidesReply>(
      flashbirdUrl, 
      JSON.stringify({"query":"query Rides{user{devices{rides{id name startTime endTime startLocation endLocation distance}}}}",
        "variables":{},
        "operationName":"Rides"})
    ).pipe(
      map((response) => {
  ...
  public getRide(id: string): Observable<Ride> {
    return this.httpClient.post<GetRideReply>(
      flashbirdUrl, 
      JSON.stringify({"query":"query Logs($rideId: ID!) { user { ride(id: $rideId) { logs { latitude longitude gpsTimestamp speed tilt } id name startTime endTime startLocation endLocation distance}}}",
        "variables":{"rideId":id},
        "operationName":"Logs"})
    ).pipe(
      map((response) => {
  ...
  public export(id: string): Observable<boolean> {
    return this.httpClient.post<ExportReply>(
      flashbirdUrl, 
      JSON.stringify({"query":"mutation ExportToGPX($exportToGpxId: ID!) { exportToGPX(id: $exportToGpxId) }",
